...
func TestGenerateInvocationScript(t *testing.T) {
	...
	// Each package gets its own staging directory under the output dir.
	assert.Assert(t, strings.Contains(script, "mkdir -p '"+outputDir+"/foo'"))
	...
	// Files are copied (not moved) into the per-package directory by their
	// resolved name.
	assert.Assert(t, strings.Contains(script, "cp -r 'bin/foo.exe' '"+outputDir+"/foo/foo.exe'"))
...
func TestGenerateInvocationScriptPermissions(t *testing.T) {
	...
	assert.Assert(t, strings.Contains(script, "cp -r 'sub/dir/foo.exe' '"+outputDir+"/foo/foo.exe'"))
	assert.Assert(t, strings.Contains(script, "chmod 755 '"+outputDir+"/foo/foo.exe'"))
}
